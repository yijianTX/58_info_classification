...
class Pretreatment(object):
    ...
    def load_data(self):
        ...
        for file in data_files:
            max_line = max(max_line, int(check_output(["wc", "-l", file]).split()[0]))
        # 数据打平后每个类目下的数据个数。
        self.max_line = int(0.05 * max_line)
        self.x = []
        self.y = []
        for file in data_files:
            with open(file) as f:
                ...
                if self.use_segment:
                    ...
                    temp_x.append([item.word.strip() for item in _ if len(item.word.strip()) and item.flag in self.pos_set and item.word not in self.stopwords_set])
                else:
                    _ = d_first["title"] + d_first["content"]
                    temp_x.append([item.strip() for item in list(_) if len(item.strip()) and item not in self.stopwords_set])
                ...
                for line in f:
                    ...
                    if self.use_segment:
                        ...
                        temp_x.append([item.word.strip() for item in temp if len(item.word.strip()) and item.flag in self.pos_set and item.word not in self.stopwords_set])
                    else:
                        temp = d_temp["title"] + d_temp["content"]
                        temp_x.append([item.strip() for item in list(temp) if len(item.strip()) and item not in self.stopwords_set])

                # 打平数据
                rep_times = int(self.max_line / len(temp_x))
                temp_x.extend(temp_x * ((rep_times - 1) if rep_times >= 1 else 0))
                temp_y.extend(temp_y * ((rep_times - 1) if rep_times >= 1 else 0))
